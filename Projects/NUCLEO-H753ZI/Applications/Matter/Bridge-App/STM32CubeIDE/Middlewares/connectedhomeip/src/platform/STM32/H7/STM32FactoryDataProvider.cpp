...
//#include <platform/STM32/ScopedNvsHandle.h>

#include <lib/core/CHIPError.h>
#include <lib/support/BytesToHex.h>
#include <lib/support/Span.h>
#include <platform/STM32/H7/CHIPDevicePlatformConfig.h>
...
CHIP_ERROR STM32FactoryDataProvider::GetSetupDiscriminator(uint16_t & setupDiscriminator)
{
...
CHIP_ERROR STM32FactoryDataProvider::GetSpake2pIterationCount(uint32_t & iterationCount)
{
//    return STM32Config::ReadConfigValue(STM32Config::kConfigKey_Spake2pIterationCount, iterationCount);
...
CHIP_ERROR STM32FactoryDataProvider::GetSpake2pSalt(MutableByteSpan & saltBuf)
{
...
CHIP_ERROR STM32FactoryDataProvider::GetSpake2pVerifier(MutableByteSpan & verifierBuf, size_t & verifierLen)
{
...
CHIP_ERROR STM32FactoryDataProvider::GetCertificationDeclaration(MutableByteSpan & outBuffer)
{
...
CHIP_ERROR STM32FactoryDataProvider::GetFirmwareInformation(MutableByteSpan & out_firmware_info_buffer)
{
    // We need a real example FirmwareInformation to be populated.
    out_firmware_info_buffer.reduce_size(0);

    return CHIP_NO_ERROR;
}

CHIP_ERROR STM32FactoryDataProvider::GetDeviceAttestationCert(MutableByteSpan & outBuffer)
{

	// DAC is Hardcoded temporarily, will read from flash/efuse in the future
...
CHIP_ERROR STM32FactoryDataProvider::GetProductAttestationIntermediateCert(MutableByteSpan & outBuffer)
{

	 // PAI is Hardcoded temporarily, will read from flash/efuse in the future
...
CHIP_ERROR STM32FactoryDataProvider::SignWithDeviceAttestationKey(const ByteSpan & messageToSign, MutableByteSpan & outSignBuffer)
{

	// DAC keys are Hardcoded temporarily, will read from flash/efuse in the future
...
// we need to implement if CHIP_DEVICE_CONFIG_ENABLE_DEVICE_INSTANCE_INFO_PROVIDER = 1
#if CHIP_DEVICE_CONFIG_ENABLE_DEVICE_INSTANCE_INFO_PROVIDER
CHIP_ERROR STM32FactoryDataProvider::GetVendorName(char * buf, size_t bufSize)
{
...
CHIP_ERROR STM32FactoryDataProvider::GetVendorId(uint16_t & vendorId)
{
...
CHIP_ERROR STM32FactoryDataProvider::GetProductName(char * buf, size_t bufSize)
{
...
CHIP_ERROR STM32FactoryDataProvider::GetProductId(uint16_t & productId)
{
    ChipError err   = CHIP_NO_ERROR;
    uint32_t valInt = 0;
...
CHIP_ERROR STM32FactoryDataProvider::GetHardwareVersionString(char * buf, size_t bufSize)
{
    size_t hardwareVersionStringLen = 0; // without counting null-terminator
    return STM32Config::ReadConfigValueStr(STM32Config::kConfigKey_HardwareVersionString, buf, bufSize, hardwareVersionStringLen);
}

CHIP_ERROR STM32FactoryDataProvider::GetRotatingDeviceIdUniqueId(MutableByteSpan & uniqueIdSpan)
{
