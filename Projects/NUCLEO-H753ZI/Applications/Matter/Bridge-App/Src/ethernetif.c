...
#include "lwip/sys.h"
#include "lwip/stm32/arch/sys_arch.h"
...
#define INTERFACE_THREAD_STACK_SIZE ( 400 )
...
void HAL_ETH_MspInit(ETH_HandleTypeDef* ethHandle)
{
  ...
  /* USER CODE END ETH_MspInit 0 */
    ...
    */
    ...
    GPIO_InitStruct.Pin = RMII_REF_CLK_Pin|RMII_MDIO_Pin|RMII_MII_CRS_DV_Pin;
    GPIO_InitStruct.Mode = GPIO_MODE_AF_PP;
    GPIO_InitStruct.Pull = GPIO_NOPULL;
    GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_HIGH;
    GPIO_InitStruct.Alternate = GPIO_AF11_ETH;
...
void HAL_ETH_MspDeInit(ETH_HandleTypeDef* ethHandle)
{
  ...
  /* USER CODE END ETH_MspDeInit 0 */
    ...
    */
    ...
    HAL_GPIO_DeInit(GPIOA, RMII_REF_CLK_Pin|RMII_MDIO_Pin|RMII_MII_CRS_DV_Pin);
...
{
    if ( READ_BIT(heth->DMAErrorCode, ETH_DMACSR_RBU) == ETH_DMACSR_RBU )
    {
         ...
         */
        __HAL_ETH_DMA_DISABLE_IT(heth, ETH_DMACIER_RBUE);
    }
}

static void ethernetif_low_errhandle(ETH_HandleTypeDef *heth)
{
    if ( READ_BIT(heth->DMAErrorCode, ETH_DMACSR_RBU) == ETH_DMACSR_RBU )
    {
        CLEAR_BIT(heth->DMAErrorCode, ETH_DMACSR_RBU);
//        NET_DBG_INFO("DMA underflow\n");
        /* Re-enable the Receive Buffer Unavailable interrupt */
        __HAL_ETH_DMA_ENABLE_IT(heth, ETH_DMACIER_RBUE);

        /* Clear RBUS ETHERNET DMA flag */
        __HAL_ETH_DMA_CLEAR_FLAG(heth, ETH_DMACSR_RBU);
...
/* USER CODE BEGIN PHY_PRE_CONFIG */
  ...
  __HAL_ETH_DMA_ENABLE_IT(&heth, ETH_DMACIER_RBUE /*| ETH_DMACIER_TBUE*/);
...
#if 0
    /* Do not sync the cache on non-cached regions */
...
//{
//	memcpy( buf, &( netif->hwaddr ), ETHARP_HWADDR_LEN );
//
//}

//( void ) memcpy( &( gnetif->hwaddr ), &( netif->hwaddr ), ETHARP_HWADDR_LEN );
